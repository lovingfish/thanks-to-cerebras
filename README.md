...
- CORS 处理与流式透传
  ...
  - 代理不消费/拼接上游返回流，直接把 `apiResponse.body`
    透传给客户端，降低延迟与内存占用
  - 对请求大小、messages 数量、content 长度和 `max_tokens` 做边界校验
  - 上游错误响应统一脱敏，避免透出账户、限流或诊断细节
  - 对上游流式响应做总时长、idle、字节数和并发限制
- Cerebras API 密钥池管理
  - 按请求轮换 key，把负载平均摊到多个 key 上
  - 支持单个密钥与模型测活，方便维护 key 池
- 代理访问密钥分发
  - 默认拒绝未授权代理请求，创建代理密钥后用 Bearer token 访问
  - 如需公开模式，必须在管理面板显式开启并承担额度风险
  - 使用 Deno KV atomic/TTL 对代理请求做全局、单代理密钥和未授权请求限流
- 模型池轮询
  - 自动切换模型，进一步分散负载
